...
public class ProductDBAccess implements ProductDAO {
    ...
    public ArrayList<ProductLowStockInfo> findByLowStock() throws DAOException, InvalidValueException {
        String sqlInstruction = "select p.id as 'product_id', p.name as 'product_name', sum(s.quantity) as 'stock_quantity', p.min_quantity as 'product_min_quantity' from product p left join stock s on p.id = s.product group by p.id having p.min_quantity is not null and stock_quantity < product_min_quantity || stock_quantity is null";
        try {
            Connection connection = SingletonConnection.getInstance();
            PreparedStatement preparedStatement = connection.prepareStatement(sqlInstruction);
